...
from dielectric_models import cauchy
...
def RT_rough_C(lam_vac, theta, params, args):
...
def SE_C(lam_vac, theta, params, args):
...
def SE_rough_C(lam_vac, theta, params, args):
...
def error_RT(params, *args):
    ...
    d_s_RT, theta_0_T, theta_0_R, c_list, lam_vac_RT, R_data, T_data, subs, alpha_R, alpha_T = args

    model_values = np.array([model_RT(lam_vac, params, (d_s_RT,
                                                     theta_0_T, theta_0_R,
...
def error_SE(params, *args):
    ...
    for theta in theta_values:
        ...
        psi_values, delta_values = np.array([model_SE_rough(lam_vac, theta, params, (subs)) for lam_vac in lam_vac_SE]).T
...
def error_tot(params, *args):
    ...
    R_values, T_values = np.array([model_RT(lam_vac, params, (d_s_RT,
                                                     theta_0_T, theta_0_R,
...
def load(sample, theta_values, minimal, maximal):
    lam_vacsubs_RT_values, M_RT_real, M_RT_imag = ld.load_nksubsdata('Glass')
    lam_vac_RT, R_data, T_data = ld.load_data(sample,minimal, maximal)
    psis_data = []
    deltas_data = []
    for theta in theta_values:
        lam_vac_SE, psi_data, delta_data = ld.load_psidelta(sample,
            minimal, maximal, theta)
...
def main():
    ...
    (lam_vacsubs_values, M_real, M_imag,
     lam_vac_RT, R_data, T_data,
     lam_vac_SE, psis_data, deltas_data) = load(sample, theta_values, minimal, maximal)
    ...
    initial_guess = np.array([2.5,10000,200])
    ...
    values = [cauchy(632, opt_param.x[:2])[2:4], cauchy(800, opt_param.x[:2])[2:4]]
